...
func init(p_plugin: EditorPlugin) -> void:
	if not p_plugin:
		push_error(tr("res://addons/fsharp_tools/fsharp_setup_dialog.gd, line 9: Invalid EditorPlugin reference passed to FSharpSetupDialog. Please open an issue at https://github.com/willnationsdev/godot-fsharp-tools."))
		return
	...
	# warning-ignore:return_value_discarded
	connect("confirmed", plugin, "_on_tool_create_fsharp_script_pressed")
